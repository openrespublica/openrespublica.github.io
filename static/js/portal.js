...
function initLockButton() {
    document.getElementById('lockBtn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            // Expected: the server fires SIGINT after returning 200, so a connection
            // reset is normal. We still proceed to show the locked screen.
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                showResult('uploadResult', true, 'Document Anchored to TruthChain',
                    { 'File': file.name, 'Type': docType, 'Status': 'Anchored · Publishing to ledger...' },
                    blob, ctrl);
            } else {
...
function initIngestForm() {
    document.getElementById('loadSampleBtn')?.addEventListener('click', () => {
        document.getElementById('philidPayload').value = JSON.stringify({
            subject: {
                fName: 'Juan', mName: 'dela', lName: 'Cruz', Suffix: '',
                sex: 'Male', DOB: '1990-01-15', POB: 'Dumaguete City, Negros Oriental',
    ...
    document.getElementById('ingestForm')?.addEventListener('submit', async e => {
        ...
        [1, 2, 3, 4].forEach((s, i) =>
            setTimeout(() => setProgress('is', s, 5), [100, 700, 2200, 4000][i])
        );
        ...
        try {
            const res = await fetch('/ingest', {
                ...
                body: JSON.stringify({
                    ...
                    purok:   document.getElementById('ingestPurok').value,
                    purpose: document.getElementById('ingestPurpose').value,
                }),
            });
...
async function loadLedger() {
    ...
    try {
        ...
        lData  = (await res.json()).reverse();  // newest first
        lPage  = 1;
        renderLedgerTable();
    } catch {
        document.getElementById('ledgerBody').innerHTML =
            '<tr><td colspan="6" style="text-align:center;padding:1.5rem;color:var(--muted);">' +
            'Could not load manifest.</td></tr>';
    }
}
...
async function loadDashboard() {
    try {
        ...
        if (!res.ok) return;
        const data = await res.json();  // already newest-first
    ...
    } catch { /* silently ignore dashboard fetch failures */ }
}
